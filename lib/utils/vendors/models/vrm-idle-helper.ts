...
import { clampByRadian } from "../three-helpers";
...
const leftShoulderBreathRotation = getQuaternionFromAngle(
  0,
  0,
  SHOULDER_BREATH_ANGLE
);
const leftArmBreathRotation = getQuaternionFromAngle(
  0,
  0,
  ARM_IDLE_ANGLE - SHOULDER_BREATH_ANGLE
);
const righArmIdleRotation = getQuaternionFromAngle(0, 0, -ARM_IDLE_ANGLE);
const rightFingerIdleRotation = getQuaternionFromAngle(
  0,
  0,
  -FINGER_IDLE_ANGLE
);
const rightThumbIdleRotation = getQuaternionFromAngle(0, -THUMB_IDLE_ANGLE, 0);
const rightShoulderBreathRotation = getQuaternionFromAngle(
  0,
  0,
  -SHOULDER_BREATH_ANGLE
);
const righArmBreathRotation = getQuaternionFromAngle(
  0,
  0,
  SHOULDER_BREATH_ANGLE - ARM_IDLE_ANGLE
);
...
function updateHead(model: VRM, deltaTime: number) {
  const bone = model.firstPerson?.humanoid.getNormalizedBoneNode(
    BoneNames.Head
  );
  if (bone) {
    // matrix.lookAt(
      ...
      rotation
        ...
        .multiply(
          rotation3.setFromRotationMatrix(
            bone.parent?.matrixWorld || matrix.identity()
          )
        ),
      Math.min(
...
function updateEyeBlink(model: VRM, deltaTime: number) {
  ...
  model.expressionManager.setValue(
    ...
    v > LOOK_CAMERA_THRESHOLD
      ? 0
      : (MathUtils.pingpong(-v, BLINK_DURATION / 2) * 2) / BLINK_DURATION
  );
}
