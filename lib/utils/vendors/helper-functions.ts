...
export function blob2ArrayBuffer(blob: Blob) {
  return new Promise<ArrayBuffer>((resolve, reject) =>
    lazyInit<typeof FileReader>(FileReader, null, {
...
export function getOrCreate(
  ...
  const entry =
    ctor == null ? Object.create(null) : Reflect.construct(ctor, args);
  map.set(key, entry);
  return entry;
}
...
export function addEventListenerAndCache(
  ...
  const cacheEntry: [EventTarget, string, EventListenerOrEventListenerObject] =
...
export function isArrayLike(src: any): src is ArrayLike<any> {
  if (src == null || typeof src === "function") return false;
...
export function forEach<S extends Iterable<any> | ArrayLike<any>>(
  src: S,
  callback: (
    value: S extends Iterable<infer T>
      ? T
      : S extends ArrayLike<infer T>
      ? T
      : unknown,
    value2: S extends Iterable<infer T>
      ? T
      : S extends ArrayLike<any>
      ? number
      : unknown,
    src: S
  ) => void
...
export function forEach(
  ...
  if (src instanceof Set) src.forEach(callback, thisArg);
  else if (isIterable(src))
    for (const entry of src) callback.call(thisArg, entry, entry, src);
  else Array.prototype.forEach.call(src, callback, thisArg);
}

export function defer(callback: Callback, thisArg: any, args: any[]) {
  return new Promise<any>((r) =>
    r(Function.prototype.apply.call(callback, thisArg, args))
  );
}
...
export function sanitize(raw: any, filter?: any) {
  ...
  for (const key in filter || raw) {
    ...
    if (typeof value.length === "number") {
      let childFilter: any;
      if (filter) {
        if (Array.isArray(filter[key])) childFilter = filter[key][0];
        else if (filter[key] !== true) continue;
      }
      const result: any[] = [];
    ...
    if (typeof value[Symbol.iterator] === "function") {
      ...
      const result: any[] = [];
      for (const entry of value) result.push(sanitize(entry, childFilter));
      sanitized[key] = result;
      continue;
    }
